...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
        ...
        // Workspace file links open the file viewer (focus stays in composer).
    ...
    fn on_transcript_event(
        ...
        match event {
            TranscriptEvent::OpenWorkspaceFile(path) => {
                let path = path.clone();
                self.file_viewer
    ...
    fn schedule_save(&mut self, cx: &mut Context<Self>) {
        ...
        self.save_task = Some(cx.spawn(async move |this, cx| {
            ...
            let Ok(snapshot) = this.update(cx, |shell, cx| {
                ...
                shell.settings.markdown_serif = crate::appearance::markdown_serif(cx);
